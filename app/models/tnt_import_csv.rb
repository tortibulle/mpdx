class TntImportCsv < TntImport
  ...
  def import_contacts
    Contact.transaction do
      ...
      lines.each do |line|
        donor_accounts, contact = add_or_update_donor_accounts(line)
        ...
        if true?(line['Is Organization'])
          ...
          donor_accounts.each do |donor_account|
            ...
            if line['Spouse First/Given Name'].present?
              ...
              spouse, contact_spouse = add_or_update_spouse(line, donor_account, contact)
  ...
  def add_or_update_company(line, donor_account)
  ...
  def add_or_update_primary_contact(line, donor_account, contact)
    ...
    add_or_update_person(line, donor_account, remote_id, contact, '')
  end

  def add_or_update_spouse(line, donor_account, contact)
    remote_id = "#{donor_account.account_number}-2"
    add_or_update_person(line, donor_account, remote_id, contact, 'Spouse ')
  end

  def add_or_update_person(line, donor_account, remote_id, contact, prefix = '')
