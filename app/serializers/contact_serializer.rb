class ContactSerializer < ActiveModel::Serializer
  ...
  def attributes
    ...
    if scope.is_a?(Hash) && scope[:since]
      hash[:deleted_people] = Version.where(item_type: 'Person', event: 'destroy', related_object_type: 'Contact', related_object_id: id)
                                     .where('created_at > ?', Time.at(scope[:since].to_i))
                                     .pluck(:item_id)
      hash[:deleted_addresses] = Version.where(item_type: 'Address', event: 'destroy', related_object_type: 'Contact', related_object_id: id)
                                        .where('created_at > ?', Time.at(scope[:since].to_i))
                                        .pluck(:item_id)
    end
